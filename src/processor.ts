...
export class D2Processor {
  plugin: D2Plugin
  debouncedMap: Map<string, (source: string, el: HTMLElement, ctx: MarkdownPostProcessorContext, signal: AbortSignal) => Promise<void>>
  ...
  attemptExport = async (source: string, el: HTMLElement, ctx: MarkdownPostProcessorContext) => {
    el.createEl('h6', {
  ...
  export = async (source: string, el: HTMLElement, ctx: MarkdownPostProcessorContext, signal: AbortSignal) => {
    try {
  ...
  getSeed(source: string) {
    const reg = source.match(/# \(seed=(.+)\)/)
    if (reg && reg[1] && !isNaN(Number(reg[1]))) {
      return Number(reg[1])
    } else {
      return 1
    }
  ...
  async generatePreview(source: string, signal: AbortSignal): Promise<string> {
    ...
    const seed = this.getSeed(source)
