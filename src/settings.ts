...
export class D2SettingsTab extends PluginSettingTab {
  ...
  addTALASettings() {
    ...
    new Setting(talaSettings)
      .setName('API token')
      .setDesc('To use TALA, copy your API token here or in ~/.local/state/tstruct/auth.json under the field "api_token"')
      .addText((text) =>
        text
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc('Available layout engines include "dagre", "ELK", and "TALA" (TALA must be installed separately from D2)')
    ...
    new Setting(containerEl)
      .setName('Theme ID')
      .setDesc('Available themes are located at https://github.com/terrastruct/d2/tree/master/d2themes')
      .addText((text) =>
        text
    ...
    new Setting(containerEl)
      ...
      .addText((text) =>
        text
          .setPlaceholder(String(DEFAULT_SETTINGS.sketch))
          .setValue(String(this.plugin.settings.sketch))
          .onChange(async (value) => {
            if (value === undefined) {
              this.plugin.settings.sketch = DEFAULT_SETTINGS.sketch
            } else {
